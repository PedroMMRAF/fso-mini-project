...
static int disk_open_nc(const char *filename, unsigned int n)
{
    ...
    // create or re-create
    if ((dfd = open(filename, O_RDWR | O_CREAT | O_TRUNC, 0666)) == -1)
        return -1;

    memset(buf, 0, DISK_BLOCK_SIZE);
    for (int i = 0; i < n; i++)
        if (write(dfd, buf, DISK_BLOCK_SIZE) == -1)
            return -1;
...
static int disk_read_nc(unsigned int blknmbr, unsigned char *buf)
{
    ...
    if (blknmbr >= nblocks)
        return -ENOSPC;
    if (lseek(dfd, blknmbr * DISK_BLOCK_SIZE, SEEK_SET) == -1)
        return -1;
    if (read(dfd, buf, DISK_BLOCK_SIZE) < DISK_BLOCK_SIZE)
        return -1;
